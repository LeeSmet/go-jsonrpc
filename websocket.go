...
type reqestHandler interface {
	handle(ctx context.Context, state State, req request, w func(func(io.Writer)), rpcError rpcErrFunc, done func(keepCtx bool), chOut chanOut)
}
...
func (c *wsConn) handleCall(ctx context.Context, state State, frame frame) {
	if c.handler == nil {
...
func (c *wsConn) handleFrame(ctx context.Context, state State, frame frame) {
...
func (c *wsConn) frameExecutor(ctx context.Context, state State) {
	for {
		select {
		case <-ctx.Done():
			return
...
func (c *wsConn) handleWsConn(ctx context.Context, state State) {
	ctx, cancel := context.WithCancel(ctx)
	defer cancel()
